    ...
    private static void unlinkSubcommand(@NotNull SlashCommandInteractionEvent event) {
        ...
        if(option == null) {
            event.getHook().editOriginal(DiscordLink.getInstance().getDiscordConfig().getString(
                    "commands.admin.subcommands.unlink.responses.invalid_user", "Invalid user account provided.")).queue();
            return;
        }
        ...
        if(!DiscordLink.getInstance().getDatabaseManager().isDiscordAccountLinked(member.getId())) {
            event.getHook().editOriginal(DiscordLink.getInstance().getDiscordConfig().getString(
                    "commands.admin.subcommands.unlink.responses.not_linked", "Selected user's account is not linked.")).queue();
            return;
        }
        ...
        event.getHook().editOriginal(DiscordLink.getInstance().getDiscordConfig().getString(
                "commands.admin.subcommands.unlink.responses.success", "Selected user's has been unlinked successfully.")).queue();
    }
